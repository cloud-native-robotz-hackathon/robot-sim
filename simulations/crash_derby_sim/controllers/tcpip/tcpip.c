...
static WbDeviceTag led[6];
static WbDeviceTag ts[2];
...
//   char text[32];
  ...
  char ts_names[2][18] = {"front bump sensor", "back bump sensor"};
  for (i = 0; i < 2; i++) {
    ts[i] = wb_robot_get_device(ts_names[i]);
    wb_touch_sensor_enable(ts[i], TIMESTEP);
  }
  ...
  led[0] = wb_robot_get_device("led 1");
  led[1] = wb_robot_get_device("led 2");
  led[2] = wb_robot_get_device("led 3");
  led[3] = wb_robot_get_device("led 4");
  led[4] = wb_robot_get_device("led 5");
  led[5] = wb_robot_get_device("led 6");
  ...
  printf("Srobo robot has been initialized by Webots\n");
...
static void run() {
  ...
  } else if (buffer[0] == 'T') { /* read touch sensor */
    sprintf(buffer, "n,%f,%f\r\n", 
            (double)wb_touch_sensor_get_value(ts[0]),
            (double)wb_touch_sensor_get_value(ts[1]));
    send(fd, buffer, strlen(buffer), 0);
